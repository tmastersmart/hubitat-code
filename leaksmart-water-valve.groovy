...
Warning:
These valves test themselves once a year but dont actualy test that the valve went off.
The valve does not actualy have a sensor that can tell the position of the valve open or closed it uses 
the motor stoping when it hits the stop as a sensor. This is a flaw in the design as the valve can stick
and the motor will think its at the stop.
...
//manufacturer :WAXMAN Model: House Water Valve - MDL-TBD  Firmware: 113B-03E8-0000001D softwareBuild: 0000001D
	...
	input name: "calBat",       type: "bool", title: "Calculate Bat%", description: "Use custom bat % or firmware report", defaultValue: true
    }
...
//  This looks at all events and decodes them CATCHALL:       
   ...
   if (descriptionMap) {
       logging ("${device} : Cluster:${descriptionMap.cluster} Command:${descriptionMap.command} Value:${descriptionMap.value} ","debug") 
       logging ("${device} : ${descriptionMap}","trace")  
   }
	def result = []
	def evt = zigbee.getEvent(description) // test for known events by the hub drivers
    if (evt) {
     logging ("${device} : Received Event: ${evt}","debug")
...
// valve status        
        if (evt.name == "switch") {
            ...
            sendEvent(name: "valve", value: val2, isStateChange: true, displayed: true) 
...
//          sendEvent(name: "lastPollD",value: new Date().format("MMM dd yyyy hh:mm", location.timeZone))
            logging ("${device} : Valve: ${val2} Water:${val3} Switch:${evt.value} Contact:${val2}","debug")
            logging ("${device} : Valve: ${val2}","info")
		}
...
//          sendEvent(name: "lastPollD",value: new Date().format("MMM dd yyyy hh:mm", location.timeZone))
            ...
            logging ("${device} : Reported battery:${value}%","info")
        }
...
//  voltage status      
  if (evt.name == "batteryVoltage") {
        ...
        sendEvent(name: "batteryVoltage", value: batteryVoltage    ,unit:"V",isStateChange: true,displayed: true) 
//      sendEvent(name: "lastPoll",value: new Date().time, isStateChange: true, displayed: false)
//      sendEvent(name: "lastPollD",value: new Date().format("MMM dd yyyy hh:mm", location.timeZone))
...
// Some models give false reports so we have to detect them    
   if (evt.name == "powerSource"){
		...
		if (val4=="mains"){
          ...
          if (state.badSupplyFlag){logging ("${device} : Bat discharging","info")}
...
private getBatteryReport() {
    logging ("${device} : get Battery Report","debug")
